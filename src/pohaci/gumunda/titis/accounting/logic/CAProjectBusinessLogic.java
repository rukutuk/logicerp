 ...
 * 
 */
public class CAProjectBusinessLogic extends TransactionBusinessLogic {

	private Employee owner;

	public CAProjectBusinessLogic(Connection connection,
			long sessionId, Employee owner) {
		super(connection, sessionId);
		this.owner = owner;
		entityMapper = MasterMap.obtainMapperFor(PmtCAProject.class);
	...
	private boolean isOutstanding(PmtCAProject pmt) {
		GenericMapper mapper = MasterMap.obtainMapperFor(ExpenseSheet.class);
		mapper.setActiveConn(connection);
		List list = mapper.doSelectWhere("PMTCAPROJECT=" + pmt.getIndex()
				+ " AND STATUS=3");
		return list.size() == 0;
	}
