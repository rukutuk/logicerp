...
public class MJ_NSProject {
	
	public MJ_NSProject(MemJournalNonStrd entity,Connection conn,Currency basecurr) {
		try {			
			String filename = "MJ_NSProject.jrxml";      
			...
			if (entity.getJournal()!=null)
				parameters.put("param_journal_type", entity.getJournal().toString());
			else
				parameters.put("param_journal_type", "");
			parameters.put("param_client", String.valueOf(entity.getProject().getCustname()));
			
			parameters.put("param_activity_code", entity.getProject().getActivity().getName());
			...
			parameters.put("param_work_description", entity.getProject().getWorkDescription());
			...
			for(int i=0;i<maxloop;i++){
				...
				if (detail.getBalanceCode().equalsIgnoreCase(Account.STR_DEBET)){
					...
					if (curr.equals(currSymbol) || currSymbol.equals(""))
