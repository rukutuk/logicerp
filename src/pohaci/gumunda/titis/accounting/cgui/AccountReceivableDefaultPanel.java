...
public class AccountReceivableDefaultPanel extends JPanel implements ActionListener {
	...
	public AccountReceivableDefaultPanel(Connection conn, long sessionid) {
		m_conn = conn;
	...
	void initData() {
		try {
			...
			m_account = logic.getAccountReceivableDefault(m_sessionid, IDBConstants.MODUL_MASTER_DATA);
	...
	void save() {
		...
		try {
			AccountingBusinessLogic logic = new AccountingBusinessLogic(m_conn);
			logic.createAccountReceivableDefault(m_sessionid, IDBConstants.MODUL_MASTER_DATA, account);
		}
		catch(Exception ex) {
			JOptionPane.showMessageDialog(this, ex.getMessage(),
