...
import pohaci.gumunda.titis.accounting.entity.reportdesign.IncomeStatementReportContext;
 ...
 * 
 */	
public class IncomeStatementReportPanel extends JPanel {
	...
	public IncomeStatementReportPanel(Connection connection, long sessionId) {
		super();
	...
	private void preview() {
		IncomeStatementDesign design = (IncomeStatementDesign) getReportDesignComboBox()
				.getSelectedItem();
		...
		try {
			IncomeStatementLogic logic = new IncomeStatementLogic(connection, sessionId);
			design = logic.getIncomeStatementDesign(design);
	...
	private String createJournals(IncomeStatementDesign design) {
	...
	private void revalidateReport(IncomeStatementReportContext context, IncomeStatementDesign design) {
		if(report==null)
			report = getReport();
		
		rightPanel.remove(report);
		
		Income_statement stmt = new Income_statement(context, design, connection, m_excel);
		if(!m_excel)
			report = stmt.getViewer();
