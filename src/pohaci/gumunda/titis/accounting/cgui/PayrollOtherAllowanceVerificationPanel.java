...
public class PayrollOtherAllowanceVerificationPanel extends
	...
	private RptSummaryOtherAllowance m_beginjasper, m_addjasper;

	public PayrollOtherAllowanceVerificationPanel(Connection conn,
			long sessionid) {
		m_conn = conn;
	...
	private void getJournalStandardSettings() {
		...
		List list = helper
				.getJournalStandardSettingWithAccount(IConstants.PAYROLL_VERIFICATION_OTHER_ALLOWANCE);
	...
	void constructComponent() {
		...
		m_componentPicker = new PayrollComponentSubmitPicker(m_conn,
				m_sessionid, PayrollComponentTree.OTHER_ALLOWANCE);
	...
	public void getSummaryreport() {
		try {
			...
			m_beginjasper = new RptSummaryOtherAllowance(m_conn, m_sessionid,
					this);
			m_addjasper = new RptSummaryOtherAllowance(m_conn, m_sessionid,
					this);
			...
			while (iterator.hasNext()) {
				...
				for (int i = 0; i < employeePayrollSubmit.length; i++) {
					if (!map.containsKey(new Long(employeePayrollSubmit[i]
						...
						dataList2.add(employeePayrollSubmit[i]);
					}
				}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == m_excellBt) {
			// pohaci.gumunda.titis.application.OtherAllowenceSubmitJasper
			// jasper =
			// new
			// pohaci.gumunda.titis.application.OtherAllowenceSubmitJasper(false);
	...
	private void dataVerification() {
		...
		try {
			...
			m_accLogic.createPayrollTransactionData(m_sessionid,
					IDBConstants.MODUL_MASTER_DATA, transaction, transDetail, employeePayrollSubmit[0]);
		} catch (Exception e) {
	...
	protected void presentingSubmittedData() {
		...
		try {
			employeePayrollSubmit = m_hrmLogic.getEmployeePayrollSubmit(
					m_sessionid, IDBConstants.MODUL_MASTER_DATA,
					m_employeepayrollSubmit);
		} catch (Exception e1) {
			// e1.printStackTrace();
		}
		if (employeePayrollSubmit != null && employeePayrollSubmit.length > 0) {
	...
	private void setPayrollSubmitInitialisation(
		...
		employeepayrollSubmit.setPayrollType(PayrollSubmitType.OTHER_ALLOWANCE);
	...
	public void presentingDataSummary(EmployeePayrollSubmit[] empPaySub,
			...
			if (empPaySub[i] != null) {
				...
				if (colIdx != -1) {
					...
					if (value != -1) {
						total[colIdx] += value;
						content = new Double(value);
						val = formatDesimal.format(content);
						// PayrollComponent payComp =
						// helper.getPayrollComponent(empPaySub[i].getPayrollComponentIndex());
						// transactionDetail = new
						// TransactionDetail(payComp.getAccount(),((Double)content).doubleValue(),baseCurrency,1,
						// unit, -1);
						// transVector.add(transactionDetail);
					} else {
						val = "";
					...
					helper.addDataAtColumn(m_data, colIdx + leftMargin, val);
				}
			}
		}
		if (m_data.size() > 0) {
