...
import pohaci.gumunda.titis.accounting.cgui.report.MJ_NSOthers;
...
public class MJOthersPanel extends RevTransactionPanel implements ActionListener, PropertyChangeListener{	
	...
	private JScrollPane m_MJNonStndrdProjAccnScrollPane,m_remarksScroll,m_descriptionScroll;	
	...
	private JPanel m_buttomButtonPanel,m_topButtonPanel,m_jPanel1,m_jPanel1_1,m_jPanel1_2,m_jPanel1_2_1,
	m_jPanel1_3,m_jPanel1_3_2,m_jPanel2,m_jPanel2_1;	
	private JTextArea m_remarksTextArea,m_descriptionTextArea;	
	private MemJournalNonStrdTableOthers m_table;
	private boolean m_edit,m_new =false;
	public boolean m_isEditSet() { return m_edit; }
	public boolean m_isNewSet() { return m_new; }
	private JRadioButton m_singleRb = new JRadioButton("Single");
	private JRadioButton m_multipleRb = new JRadioButton("Multiple");
	
	public MJOthersPanel(Connection conn, long sessionid) {
		m_conn = conn;
	...
	private void initComponents() {
		JLabel remarksLbl=new JLabel();
		JLabel statusLbl = new JLabel();
		JLabel submittedDateLbl = new JLabel();
		JLabel transactionCodeLbl = new JLabel();
		JLabel transactionDateLbl = new JLabel();
		JLabel memorialJournalNoLbl = new JLabel();
		JLabel descriptionLbl = new JLabel();
		JLabel unitCodeLbl = new JLabel();
		...
		m_printViewRefNoBtn = new javax.swing.JButton(new ImageIcon("../images/filter.gif"));m_transactionDateDate = new pohaci.gumunda.titis.application.DatePicker();
		m_table = new MemJournalNonStrdTableOthers();
		...
		m_jPanel1_2.setPreferredSize(new Dimension(770, 320));
		...
		submittedDateLbl.setText("Submitted Date");
		...
		transactionCodeLbl.setText("Journal Type");
		transactionCodeLbl.setPreferredSize(new Dimension(140, 15));
		gridBagConstraints = new GridBagConstraints();
		gridBagConstraints.gridx = 0;
		gridBagConstraints.gridy = 2;
		gridBagConstraints.anchor = GridBagConstraints.WEST;
		gridBagConstraints.insets = new Insets(1, 35, 1, 1);
		m_jPanel1_2_1.add(transactionCodeLbl, gridBagConstraints);
		...
		memorialJournalNoLbl.setText("Memorial Journal No");
		...
		m_descriptionScroll.setPreferredSize(new Dimension(370, 64));
		...
		m_journaTypeCombo.setPreferredSize(new Dimension(370, 18));
		gridBagConstraints = new GridBagConstraints();
		gridBagConstraints.gridx = 1;
		gridBagConstraints.gridy = 2;
		gridBagConstraints.anchor = GridBagConstraints.WEST;
		...
		unitCodeLbl.setPreferredSize(new Dimension(140, 15));
		gridBagConstraints = new GridBagConstraints();
		gridBagConstraints.gridx = 0;
		...
		m_remarksScroll.setViewportView(m_remarksTextArea);
	...
	public void actionPerformed(ActionEvent e){
		if (e.getSource() == m_printViewRefNoBtn) {
			if (m_entity.getIndex()>0){
				new MJ_NSOthers(m_entity,m_conn,baseCurrency);
			}else{
			...
			SearchMemorialJournalDialog dlg = new SearchMemorialJournalDialog(GumundaMainFrame.getMainFrame(), 
					"Search Memorial Journal","Non Standard Journal Non Project", m_conn, m_sessionid,
					new MemJournalStrdLoader(m_conn,MemJournalNonStrd.class),"0");
			dlg.setVisible(true);
	...
	public void clearKomponen(){		
		...
		m_journaTypeCombo.setObject(null);		
	...
	private void setDefaultSignature() {		
		Signature sign = null;
		sign = Signature.getSignature(m_conn, IConstants.MJ_NONSTANDARD_OTHERS, Signature.SIGN_ORIGINATOR);
		if(sign!=null)
			m_defaultOriginator = sign.getFullEmployee();
		sign = Signature.getSignature(m_conn, IConstants.MJ_NONSTANDARD_OTHERS, Signature.SIGN_APPROVED);
		if(sign!=null)
	...
	protected boolean cekValidity() {
		...
		if (m_unitCodeComp.getObject()==null)
			addInvalid("Unit must be selected");
	...
	private void detailAccountOperation() {
		if (m_table.getRowCount()>=0){
			...
			for (int i=0;i<m_table.getDataCount();i++){
				...
				double debit = 0;	
				double credit = 0;
				debit = ((Double)m_table.getValueAt(i,4)).doubleValue();
				credit = ((Double)m_table.getValueAt(i,5)).doubleValue();				
				if (debit > 0){
	...
	protected void disableEditMode() {
		boolean cekFalse = false;
		m_statusText.setEditable(cekFalse);
		m_submittedDateText.setEditable(cekFalse);		
		m_refNoText.setEditable(cekFalse);
		m_journaTypeCombo.setEnabled(cekFalse);
		m_transactionDateDate.setEditable(cekFalse);
		m_descriptionTextArea.setEditable(cekFalse);
		m_unitCodeComp.setEnabled(cekFalse);
		m_departmentComp.setEnabled(cekFalse);
		m_originatorComp.setEnabled(cekFalse);
		m_approvedComp.setEnabled(cekFalse);
		m_addMJNonStndrdProjAccntBtn.setEnabled(cekFalse);
		m_deleteMJNonStndrdProjAccntBtn.setEnabled(cekFalse);
		m_singleRb.setEnabled(cekFalse);
		m_multipleRb.setEnabled(cekFalse);
		m_table.setEnabled(cekFalse);
	}
	
	protected void enableEditMode() {
		boolean cekTrue = true;
		m_journaTypeCombo.setEnabled(cekTrue);
		m_transactionDateDate.setEditable(cekTrue);
		m_descriptionTextArea.setEditable(cekTrue);
		m_unitCodeComp.setEnabled(cekTrue);
		m_departmentComp.setEnabled(cekTrue);
		m_originatorComp.setEnabled(cekTrue);
		m_approvedComp.setEnabled(cekTrue);		
		m_addMJNonStndrdProjAccntBtn.setEnabled(cekTrue);
		m_deleteMJNonStndrdProjAccntBtn.setEnabled(cekTrue);
		m_singleRb.setEnabled(cekTrue);
		m_multipleRb.setEnabled(cekTrue);
		m_table.setEnabled(cekTrue);		
	}
	...
	private void SetCompanyLoanToTable(MemJournalNonStrdDet detail, Vector temp1, AccountingBusinessLogic accLogic) {
	...
	//int m_lastRow;
	class MemJournalNonStrdTableOthers extends JTable {		
		private static final long serialVersionUID = 1L;
		TableModel model = new TableModel();
		ArrayList listData=new ArrayList();
		protected MemJournalNonStrdTableOthers() {			
			model.addColumn("No");
