...
import pohaci.gumunda.titis.accounting.cgui.report.AccountPayable;
import pohaci.gumunda.titis.application.DatePicker;

public class AccountPayableReport extends JPanel implements ActionListener {
	...
	public AccountPayableReport(Connection conn,long sessionid) {
		m_conn = conn;
	...
	void initComponents() {
		...
		northLeftPanel.setPreferredSize(new Dimension(350, 60)); // size northLeftPanel
		gridBagConstraints.gridx = 0; 
		gridBagConstraints.gridy = 0;
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == m_btnView){
			if (!cekValidity()) 
				return;
			if (m_date.getDate().getTime()>new Date().getTime()){
				if (JOptionPane.showConfirmDialog(this,"The date is bigger than today ,Are you sure to show the report?","",JOptionPane.YES_NO_OPTION)==0){
					getJasper(false);
				}else{
					return;
				}				
	...
	public void setJasper(){		
		AccountPayable jasper = new AccountPayable();		
		m_jrv = jasper.getPrintView();
	}
	
	public void getJasper(boolean isExcel){		
		AccountPayable jasper = new AccountPayable(m_conn,m_sessionid,m_unitPicker,m_date,isExcel);
		if (!isExcel)
			setPanel(jasper);
	}

	private void setPanel(AccountPayable jasper) {
		if (m_jrv != null){			
