...
import pohaci.gumunda.titis.accounting.cgui.report.Vcr_IOUSettledOthers;
...
public class PaymentCAIOUOthersSettledPanel extends RevTransactionPanel 
implements ActionListener,PropertyChangeListener{
	
	private static final long serialVersionUID = 1L;
	private PaymentCAIOUOthersCashReceivePanel CashReceiveGUI; 
	private PmtCAIOUOthersSettled m_entity;
	...
	String m_msg="tidak balance";
	
	public PaymentCAIOUOthersSettledPanel(JDialog owner,Connection conn, long sessionid,PmtCAIOUOthers obj,PaymentCAIOUOthersCashReceivePanel receivenya,EmployeePicker payToComp) {
		m_owner = owner;
		entityparent=obj;    	
		m_conn = conn;
		...
		setDefaultSignature(); 
		setEntity(new PmtCAIOUOthersSettled());
		m_entityMapper = MasterMap.obtainMapperFor(PmtCAIOUOthersSettled.class);
	...
	public void PertamaMuncul(PmtCAIOUOthers temp){
		try {
			...
			if (listData.length==0){ 
				...
				doLoad((PmtCAIOUOthersSettled)listData[0]);
				checkReceived();
				warning();
				HitungInstallment(temp);
			...
			AccountComp.setFilterText(entityparent.getCurrency().getSymbol());  
		}
		catch (Exception e){			
		}		
	}
	...
	public void actionPerformed(ActionEvent e){
		if(e.getSource() == m_printViewRefNoBtn) {
			if (m_entity.getIndex()>0){
				new Vcr_IOUSettledOthers(m_owner,entityparent,m_entity,m_payToComp);
			}else{
			...
			m_owner.setTitle("Cash Advance Others - I Owe You Cash Receive");
			m_owner.getContentPane().add(new PaymentCAIOUOthersCashReceivePanel(m_owner,m_conn, m_sessionid,entityparent));
	...
	protected Object createNew(){
		PmtCAIOUOthersSettled a  = new PmtCAIOUOthersSettled();
	...
	protected boolean cekBalance(){
		boolean cek = false;
		double amountSettled = ((Number)TotalSettledText.getValue()).doubleValue();
		if (amountSettled>=TotalInstallment)
			cek = true;			
		return cek;
	}
	
	protected void doSave() {
		if ((!cekValidity()) ||(!CashReceiveGUI.cekValidity())) 
			return;
		if (!cekBalance()){
	...
	void setEntity(Object m_entity) {
		PmtCAIOUOthersSettled oldEntity = this.m_entity;
		if (oldEntity!=null)
			oldEntity.removePropertyChangeListener(this);		
		this.m_entity = (PmtCAIOUOthersSettled)m_entity;
		this.m_entity.addPropertyChangeListener(this);
	}
	
	PmtCAIOUOthersSettled entity() {
		return m_entity;
	}
	...
	protected void setDefaultSignature() {
		Signature sign = null;
		sign = Signature.getSignature(m_conn, IConstants.PAYMENT_CASHADVANCE_IOU_OTHERS_SETTLEMENT, Signature.SIGN_ORIGINATOR);
		if(sign!=null)
			defaultOriginator = sign.getFullEmployee();
		sign = Signature.getSignature(m_conn, IConstants.PAYMENT_CASHADVANCE_IOU_OTHERS_SETTLEMENT, Signature.SIGN_APPROVED);
		if(sign!=null)
			defaultApproved = sign.getFullEmployee();
		sign = Signature.getSignature(m_conn, IConstants.PAYMENT_CASHADVANCE_IOU_OTHERS_SETTLEMENT, Signature.SIGN_RECEIVED);
		if(sign!=null)
