...
import pohaci.gumunda.titis.accounting.cgui.report.CashAdvance;
import pohaci.gumunda.titis.application.DatePicker;


public class CashAdvanceReport extends JPanel implements ActionListener {
	...
	private JComboBox m_CACombo;
	
	
	public CashAdvanceReport(Connection conn,long sessionid) {
		m_conn = conn;
	...
	void initComponents() {
		...
		leftPanel.add(CALbl, gridBagConstraints);
		...
		leftPanel.add(m_CACombo, gridBagConstraints);	
	...
	public void setJasper(){		
		CashAdvance jasper = new CashAdvance();		
		m_jrv = jasper.getPrintView();
	}
	
	public void getJasper(boolean isExcel){		
		CashAdvance jasper = new CashAdvance(m_conn,m_sessionid,m_unitPicker,m_date,m_CACombo.getSelectedItem().toString(),isExcel);
		if (!isExcel)
			setPanel(jasper);
	}
	
	private void setPanel(CashAdvance jasper) {
		if (m_jrv != null){			
