...
public class SearchCustomerDlg extends JDialog implements ActionListener {
  /**
	 * 
	 */
	private static final long serialVersionUID = 1L;
JFrame m_mainframe;
  CustomerListPanel m_panel;
  ...
  public SearchCustomerDlg(JFrame owner, CustomerListPanel panel,
                           Connection conn, long sessionid) {
    super(owner, "Search Customer", false);
    setSize(350, 400);
    m_mainframe = owner;
    m_panel = panel;
    m_conn = conn;
  ...
  void find() {
    ...
    try {
      ProjectBusinessLogic logic = new ProjectBusinessLogic(m_conn);
      m_panel.reset(logic.getCustomerByCriteria(m_sessionid,
  ...
  class SearchTable extends JTable {
    ...
    public String getCriterion() throws Exception {
     ...
     // String equality = " = ";
      ...
      String querytable = "FROM " + IDBConstants.TABLE_CUSTOMER;
      String addquery = "";
      ...
      if(obj instanceof CompanyGroup) {
        if(addquery.equals("")) {
          addquery = "," + IDBConstants.TABLE_CUSTOMER_GROUP;
          queryselect = "SELECT DISTINCT " + IDBConstants.TABLE_CUSTOMER + ".* ";
          querytable += addquery;
        }

        if(criteria.equals("")) {
          criteria = "WHERE " + IDBConstants.ATTR_AUTOINDEX + "=" + IDBConstants.ATTR_CUSTOMER_INDEX + operator +
                     IDBConstants.ATTR_GROUP_INDEX + " = " + ((CompanyGroup)obj).getIndex();
        }
        else {
          if(addquery.equals("")) {
            criteria += operator + IDBConstants.ATTR_AUTOINDEX + "=" + IDBConstants.ATTR_CUSTOMER_INDEX + operator +
                        IDBConstants.ATTR_GROUP_INDEX + " = " + ((CompanyGroup)obj).getIndex();
          }
          else {
