...
public class PartnerCompanyGroupPanel extends JPanel implements ActionListener  {
  /**
	 * 
	 */
	private static final long serialVersionUID = 1L;
PartnerGroupTable m_table;
  ...
  public PartnerCompanyGroupPanel(Connection conn, long sessionid) {
    m_conn = conn;
  ...
  void constructComponent() {
    System.out.println("partner group komponen");    
    JPanel buttonPanel = new JPanel();
    JPanel centerPanel = new JPanel();

    // definisikan tabel
    m_table = new PartnerGroupTable();    
  ...
  void onSave() {
    ...
    try {
      ...
      if(m_new){
        CompanyGroup group = loic.createPartnerCompanyGroup(m_sessionid, IDBConstants.MODUL_MASTER_DATA, new CompanyGroup(name, note));
        model.setValueAt(group, m_editedIndex, 0);
      }
      else if(m_edit){
        CompanyGroup group = loic.updatePartnerCompanyGroup(m_sessionid, IDBConstants.MODUL_MASTER_DATA, m_group.getIndex(), new CompanyGroup(name, note));
  ...
  void onDelete() {
    ...
    try{
      ProjectBusinessLogic logic = new ProjectBusinessLogic(m_conn);
      logic.deletePartnerCompanyGroup(m_sessionid, IDBConstants.MODUL_MASTER_DATA, group.getIndex());
    }
    catch(Exception ex){
      JOptionPane.showMessageDialog(this, ex.getMessage(),
                                    "Warning", JOptionPane.WARNING_MESSAGE);
   ...
   *
   */
  class PartnerGroupTable extends JTable {
    /**
	 * 
	 */
	private static final long serialVersionUID = 1L;

	public PartnerGroupTable() {
      PartnerGroupTableModel model = new PartnerGroupTableModel();
   ...
   *
   */
  class PartnerGroupTableModel extends DefaultTableModel implements ListSelectionListener {
    /**
	 * 
	 */
	private static final long serialVersionUID = 1L;
