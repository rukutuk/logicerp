...
public class LeaveTypePanel extends JPanel implements ActionListener  {
  /**
	 * 
	 */
	private static final long serialVersionUID = 1L;
LeaveTypeTable m_table;
  ...
  int m_editedIndex = -1;
  LeaveType m_type = null;

  public LeaveTypePanel(Connection conn, long sessionid) {
    m_conn = conn;
  ...
  void initData() {
    ...
    try {
      ...
      for(int i = 0; i < type.length;  i ++) {
        model.addRow(new Object[]{type[i], type[i].getDescription(), new Boolean(type[i].getDeduction())});
      }
  ...
  void onNew() {
    ...
    model.setValueAt(new Boolean(false), m_editedIndex, 2);
  ...
  void onEdit() {
    ...
    m_type = (LeaveType)m_table.getValueAt(m_editedIndex, 0);
  ...
  void onSave() {
    ...
    String code = "";
    if(objValue instanceof LeaveType)
      code = ((LeaveType)objValue).getCode();
    else
    ...
    boolean deduction = ((Boolean)model.getValueAt(m_editedIndex, 2)).booleanValue();

    try {
      HRMBusinessLogic logic = new HRMBusinessLogic(m_conn);

      if(m_new){
        LeaveType type = logic.createLeaveType(m_sessionid, IDBConstants.MODUL_MASTER_DATA, new LeaveType(code, desc, deduction));
        model.setValueAt(type, m_editedIndex, 0);
      }
      else if(m_edit){
        LeaveType type = logic.updateLeaveType(m_sessionid, IDBConstants.MODUL_MASTER_DATA, m_type.getIndex(), new LeaveType(code, desc, deduction));
        model.setValueAt(type, m_editedIndex, 0);
      }
    }
    catch(Exception ex){
  ...
  void onCancel() {
    ...
    if(m_new){
      ...
      m_edit = false;
      m_table.updateLeaveType(m_type, m_editedIndex);
      m_addBt.setEnabled(true);
      m_editBt.setEnabled(true);
      m_saveBt.setEnabled(false);
      m_cancelBt.setEnabled(false);
  ...
  void onDelete() {
    DefaultTableModel model = (DefaultTableModel)m_table.getModel();
    m_editedIndex = m_table.getSelectedRow();
    LeaveType type = (LeaveType)model.getValueAt(m_editedIndex, 0);
    ...
    try{
      HRMBusinessLogic logic = new HRMBusinessLogic(m_conn);
      logic.deleteLeaveType(m_sessionid, IDBConstants.MODUL_MASTER_DATA, type.getIndex());
    }
    catch(Exception ex){
      JOptionPane.showMessageDialog(this, ex.getMessage(),
                                    "Warning", JOptionPane.WARNING_MESSAGE);
   ...
   *
   */
  class LeaveTypeTable extends JTable {
    /**
	 * 
	 */
	private static final long serialVersionUID = 1L;
    ...
    public void updateLeaveType(LeaveType type, int row) {
      DefaultTableModel model = (DefaultTableModel)getModel();
      model.removeRow(row);

      model.insertRow(row, new Object[]{type, type.getDescription(), new Boolean(type.getDeduction())});
   ...
   *
   */
  class LeaveTypeTableModel extends DefaultTableModel implements ListSelectionListener {
    /**
	 * 
	 */
	private static final long serialVersionUID = 1L;
