...
EducationTable m_table;
  ...
  int m_editedIndex = -1;
  Education m_edu = null;

  public EducationPanel(Connection conn, long sessionid) {
    m_conn = conn;
  ...
  void initData() {
    ...
    try {
      ...
      Education[] edu = logic.getAllEducation(m_sessionid, IDBConstants.MODUL_MASTER_DATA);

      for(int i = 0; i < edu.length;  i ++) {
    	Education e = new Education(edu[i], Education.CODE);
        model.addRow(new Object[]{e, e.getDescription()});
      }
    }
    catch(Exception ex) {
  ...
  void onEdit() {
    ...
    m_edu = (Education)m_table.getValueAt(m_editedIndex, 0);
  ...
  void onSave() {
    ...
    String code = "";
    if(objValue instanceof Education)
      code = ((Education)objValue).getCode();
    else
      code = (String)objValue;

    if(code == null || code.equals("")){
      JOptionPane.showMessageDialog(this, "Grade of Education have to fill.");
      return;
    }

    String desc = (String)model.getValueAt(m_editedIndex, 1);
    if(desc == null || desc.equals("")){
      JOptionPane.showMessageDialog(this, "Description of Education have to fill.");
      return;
    }
    ...
    try {
      ...
      if(m_new){
        Education edu = logic.createEducation(m_sessionid, IDBConstants.MODUL_MASTER_DATA, new Education(code, desc));
        edu = new Education(edu, Education.CODE);
        model.setValueAt(edu, m_editedIndex, 0);
      }
      else if(m_edit){
        Education edu = logic.updateEducation(m_sessionid, IDBConstants.MODUL_MASTER_DATA, m_edu.getIndex(), new Education(code, desc));
        edu = new Education(edu, Education.CODE);
        model.setValueAt(edu, m_editedIndex, 0);
      }
    }
  ...
  void onCancel() {
    ...
    if(m_new){
      ...
      m_edit = false;
      m_table.updateEducation(m_edu, m_editedIndex);
      m_addBt.setEnabled(true);
      m_editBt.setEnabled(true);
      m_saveBt.setEnabled(false);
      m_cancelBt.setEnabled(false);
  ...
  void onDelete() {
    DefaultTableModel model = (DefaultTableModel)m_table.getModel();
    m_editedIndex = m_table.getSelectedRow();
    Education edu = (Education)model.getValueAt(m_editedIndex, 0);
    ...
    try{
      HRMBusinessLogic logic = new HRMBusinessLogic(m_conn);
      logic.deleteEducation(m_sessionid, IDBConstants.MODUL_MASTER_DATA, edu.getIndex());
    }
    catch(Exception ex){
      JOptionPane.showMessageDialog(this, ex.getMessage(),
                                    "Warning", JOptionPane.WARNING_MESSAGE);
  ...
  class EducationTable extends JTable {
    ...
    public void updateEducation(Education edu, int row) {
      DefaultTableModel model = (DefaultTableModel)getModel();
      model.removeRow(row);

      edu = new Education(edu, Education.CODE);
      model.insertRow(row, new Object[]{edu, edu.getDescription()});
   ...
   *
   */
  class EducationTableModel extends DefaultTableModel implements ListSelectionListener {
    /**
	 * 
	 */
	private static final long serialVersionUID = 1L;
