...
public class RptDeductionsJasper {
	JRViewer m_jrv;
	Connection m_conn;
	long m_sessionid;
	Employee m_employee;
	...
	public RptDeductionsJasper(Connection conn,long sessionid,Employee employee,String bln_thn) {
		try {
			...
			String filename = "RptDeductions.jrxml";
			if(filename.equals(""))
				return;
			...
			// untuk kelas ini, superlabel-nya yang pertama muncul alias index 1
			long superlabel = getSuperlabel(1);
			if (employee!=null){
	...
	private void tableHeader(DefaultTableModel model) {
		model.addColumn("deductionsfield");
