...
public class RptSummaryTran{
	...
	private PayrollTransportationAllowanceSubmitPanel m_panelSubmit;
	RptSummaryTranSub m_jasper =null;
	String m_filename;
	boolean m_cekverivy;
	public RptSummaryTran(Connection conn,long sessionid,PayrollTransportationAllowanceSubmitPanel panel){
		m_conn = conn;
		m_sessionid = sessionid;
		m_panelSubmit = panel;
		m_jasper= new RptSummaryTranSub(m_panelSubmit);
		m_filename = "RptSummaryTrans.jrxml";
		m_cekverivy = false;
	}
	
	PayrollTransportationAllowanceVerificationPanel m_panelVerivy;
	public RptSummaryTran(Connection conn,long sessionid,PayrollTransportationAllowanceVerificationPanel panel){
		m_conn = conn;
		m_sessionid = sessionid;
		m_panelVerivy = panel;
		m_jasper= new RptSummaryTranSub(m_panelVerivy);
		m_filename = "RptSummaryTrans.jrxml";
		m_cekverivy=true;
	}
	
	public void setRptSummarySubmit(PayrollComponent[] payrollComponents,EmployeeTransportationAllowance[] empTranSubmit,
		...
		try {
			...
			m_jasper.RptSummarySubmit(payrollComponents,empTranSubmit,payrollComponentsDefault,unitPicker.getUnit(),
	...
	public void setRptSummarySubmit2(List totalList,List deptlist,UnitPicker unitPicker,String period,String date){
		try {
			...
			JasperReport jasperReport = JasperCompileManager.compileReport("report/" + m_filename);
			Map parameters = new HashMap();
			RptSummaryTranSub jasper =null;
			jasper= new RptSummaryTranSub(m_conn,m_sessionid,totalList,deptlist);
	...
	public void setRptSummaryVerivy(PayrollComponent[] payrollComponents, EmployeeTransportationAllowance[] empMealAllowSubmit,UnitPicker unitPicker,Organization org,String period,String date) {
		try {
			m_dateFormat = new SimpleDateFormat("dd-MM-yyyy");
	...
	public void getPrintView(){
		try {
			...
			m_view.setTitle("Summary transportation report");
