...
public class RptSummaryMeal{
	...
	private PayrollMealAllowanceSubmitPanel m_panelMealSubmit;
	RptSummaryMealSub m_jasper =null;
	String m_filename;
	boolean m_cekverivy;
	public RptSummaryMeal(Connection conn,long sessionid,PayrollMealAllowanceSubmitPanel panel){
		m_conn = conn;
		m_sessionid = sessionid;
		m_panelMealSubmit = panel;
		m_jasper= new RptSummaryMealSub(m_panelMealSubmit);
		m_filename = "RptSummaryMeal.jrxml";
		m_cekverivy = false;
	}
	
	PayrollMealAllowanceVerificationPanel m_panelmealverivy;
	public RptSummaryMeal(Connection conn,long sessionid,PayrollMealAllowanceVerificationPanel panel){
		m_conn = conn;
		m_sessionid = sessionid;
		m_panelmealverivy = panel;
		m_jasper= new RptSummaryMealSub(m_panelmealverivy);
		m_filename = "RptSummaryMeal.jrxml";
		m_cekverivy=true;
	}
	
	public void setRptSummarySubmit(PayrollComponent[] payrollComponents,EmployeeMealAllowanceSubmit[] empMealAllowSubmit,
		...
		try {
			...
			m_jasper.RptSummaryMealSubmit(payrollComponents,empMealAllowSubmit,payrollComponentsDefault,unitPicker.getUnit(),
	...
	public void setRptSummarySubmit2(List totalList,List deptlist,UnitPicker unitPicker,String period,String date){
		try {
			...
			JasperReport jasperReport = JasperCompileManager.compileReport("report/" + m_filename);
			Map parameters = new HashMap();
			RptSummaryMealSub jasper =null;
			jasper= new RptSummaryMealSub(m_conn,m_sessionid,totalList,deptlist);
