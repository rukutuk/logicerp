...
public class HRMPayrollPaychequeJasper {
	PayrollPaychequeSubmitPanel m_panel;  
	JasperPrint m_jasperPrint;
  public HRMPayrollPaychequeJasper(PayrollPaychequeSubmitPanel panel,PayrollComponent[] payrollComponents,
		  Unit unit, Employee_n[] emps,
		  EmployeePayrollSubmit[] empPayrollSubmit,PayrollComponent[] payrollComponentsDefault,
		  JComboBox typecombo,boolean cekExcel) {
			  
	this.m_panel = panel;
    try {
      String filename = "RptPayrollPaychequesHrd.jrxml";          
      if(filename.equals(""))
        return;      
      ...
      tableHeader(model);
      PayrollPaychequeSubmitLogic paychequeLogic = new PayrollPaychequeSubmitLogic(panel,model,
    		  payrollComponents,emps,unit,empPayrollSubmit,payrollComponentsDefault,cekExcel);
      paychequeLogic.setType(typecombo);
      paychequeLogic.tableBodyReport();    
