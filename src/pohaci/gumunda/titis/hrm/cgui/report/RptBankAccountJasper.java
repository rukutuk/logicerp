...
public class RptBankAccountJasper {
	JRViewer m_jrv;
	Connection m_conn;
	Employee m_employee;
	long m_sessionid;
	...
	public RptBankAccountJasper(Connection conn,long sessionid,Employee employee,int loop) {
		try {
			this.m_conn = conn;
			this.m_sessionid = sessionid;
			this.m_employee = employee;
			String filename = "RptBankAccount.jrxml";
			if(filename.equals(""))
				return;
	...
	private void getNonEmpty(DefaultTableModel model,int loop) {
		...
		BankRpt[] BankRpt = null;
		try {
			BankRpt = logic.getBankRpt(m_sessionid,"",m_employee.getIndex());
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (BankRpt.length>0){
			for (int i=0;i<BankRpt.length;i++){
				model.addRow(new Object[]{
						String.valueOf(BankRpt[i].getName()),
						String.valueOf(BankRpt[i].getAccountno()),
						String.valueOf(BankRpt[i].getBank()),
						String.valueOf(BankRpt[i].getCurrency())});
			}
		}else{
			getEmpty(model);
		}
