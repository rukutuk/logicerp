...
public class RptSummaryMealSub {	
	Connection m_conn;
	long m_sessionid;
	...
	private PayrollMealAllowanceSubmitPanel m_submitpanel;
	PayrollMealAllowanceVerificationPanel m_verivypanel;
	public RptSummaryMealSub(PayrollMealAllowanceSubmitPanel panel){
		m_submitpanel = panel;
	}
	
	public RptSummaryMealSub(PayrollMealAllowanceVerificationPanel panel){
		m_verivypanel = panel;
	}
	
	public void RptSummaryMealSubmit(PayrollComponent[] payrollComponents,EmployeeMealAllowanceSubmit[] empMealAllowSubmit,
			PayrollComponent[] payrollComponentsDefault,Unit unit,Connection conn, long sessionid,Employee_n[] emp){
		m_conn=conn;
		m_sessionid = sessionid;
		try {
			String filename = "RptSummaryMealsub.jrxml";      
			m_jasperReport = JasperCompileManager.compileReport("report/" + filename);
			DefaultTableModel model = new DefaultTableModel();
			header(model);
				PayrollMealAllowanceSubmitLogic summaryLogic = new PayrollMealAllowanceSubmitLogic(m_submitpanel,model,
					payrollComponents,emp,unit,empMealAllowSubmit,payrollComponentsDefault,false);
	...
	public RptSummaryMealSub(Connection conn, long sessionid,List totalList,List deptlist){
		m_conn=conn;
		m_sessionid = sessionid;
		//DecimalFormat formatDesimal = new DecimalFormat("#,##0.00");
		double[] ttldept = {0,0};
		try {
			String filename = "RptSumDeptPayrollMeal.jrxml";      
			m_jasperReport = JasperCompileManager.compileReport("report/" + filename);
			DefaultTableModel model = new DefaultTableModel();
			headertotal(model);
