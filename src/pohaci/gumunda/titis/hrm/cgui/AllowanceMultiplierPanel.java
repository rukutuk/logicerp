...
public class AllowanceMultiplierPanel extends JPanel implements ActionListener  {
  /**
	 * 
	 */
...
AllowenceMultiplierTable m_table;
  ...
  int m_editedIndex = -1;
  AllowenceMultiplier m_multiplier = null;

  public AllowanceMultiplierPanel(Connection conn, long sessionid) {
    m_conn = conn;
  ...
  void constructComponent() {
    ...
    m_table = new AllowenceMultiplierTable();
  ...
  void initData() {
    ...
    try {
      ...
      AllowenceMultiplier[] multiplier = logic.getAllAllowenceMultiplier(m_sessionid, IDBConstants.MODUL_MASTER_DATA);

      for(int i = 0; i < multiplier.length;  i ++) {
        model.addRow(new Object[]{multiplier[i], multiplier[i].getDescription(), new Float(multiplier[i].getMuliplier())});
      }
    }
    catch(Exception ex) {
  ...
  void onEdit() {
    ...
    m_multiplier = (AllowenceMultiplier)m_table.getValueAt(m_editedIndex, 0);
  ...
  void onSave() {
    ...
    String code = "";
    if(objValue instanceof AllowenceMultiplier)
      code = ((AllowenceMultiplier)objValue).getAreaCode();
    else
    ...
    float multiply = ((Float)model.getValueAt(m_editedIndex, 2)).floatValue();
    ...
    try {
      ...
      if(m_new){
        AllowenceMultiplier multiplier = logic.createAllowenceMultiplier(m_sessionid, IDBConstants.MODUL_MASTER_DATA, new AllowenceMultiplier(code, desc, multiply));
        m_table.updateAllowenceMultiplier(multiplier, m_editedIndex);
      }
      else if(m_edit){
        AllowenceMultiplier multiplier = logic.updateAllowenceMultiplier(m_sessionid, IDBConstants.MODUL_MASTER_DATA, m_multiplier.getIndex(), new AllowenceMultiplier(code, desc, multiply));
        m_table.updateAllowenceMultiplier(multiplier, m_editedIndex);
      }
    }
    catch(Exception ex){
  ...
  void onDelete() {
    DefaultTableModel model = (DefaultTableModel)m_table.getModel();
    m_editedIndex = m_table.getSelectedRow();
    AllowenceMultiplier multiplier = (AllowenceMultiplier)model.getValueAt(m_editedIndex, 0);
    ...
    try{
      HRMBusinessLogic logic = new HRMBusinessLogic(m_conn);
      logic.deleteAllowenceMultiplier(m_sessionid, IDBConstants.MODUL_MASTER_DATA, multiplier.getIndex());
    }
    catch(Exception ex){
      JOptionPane.showMessageDialog(this, ex.getMessage(),
                                    "Warning", JOptionPane.WARNING_MESSAGE);
  ...
  class AllowenceMultiplierTable extends JTable {
	...
	public AllowenceMultiplierTable() {
      ...
      getColumnModel().getColumn(2).setCellRenderer(new FloatCellRenderer(JLabel.CENTER));
    ...
    public void updateAllowenceMultiplier(AllowenceMultiplier multiplier, int row) {
      DefaultTableModel model = (DefaultTableModel)getModel();
      model.removeRow(row);

      model.insertRow(row, new Object[]{multiplier, multiplier.getDescription(), new Float(multiplier.getMuliplier())});
   ...
   *
   */
  class AllowenceMultiplierTableModel extends DefaultTableModel implements ListSelectionListener {
    /**
	 * 
	 */
	private static final long serialVersionUID = 1L;
